/**
 * POST /api/hint — get a short, answer-safe hint for a quiz question.
 *
 * This is a Next.js App Router Route Handler. The player pages call it when a
 * student asks for help on a question. It returns a one/two-sentence nudge that
 ...
 * does its OWN cookie-based auth. Hints do NOT count toward the weekly quiz
 * limit — only quiz generations do — so we never touch that counter here.
 */

// `generateHint` builds the nudge with the Groq engine (server-only, src/lib/llm.ts).
...
/**
 * Handles POST requests to /api/hint.
 *
 * @param req - the incoming web Request ({ prompt, context? } JSON body).
 * @returns a JSON Response: { hint } on success, or { error } with a status.
 */
export async function POST(req: Request): Promise<Response> {
  ...
  // 2. Validate: body must be an object with a non-empty `prompt` string.
  //    `context` is optional and, when present, must be a string.
  ...
  if (typeof b.prompt !== 'string' || b.prompt.trim().length === 0) {
    return Response.json(
      { error: 'A question prompt is required to get a hint.' },
      { status: 400 },
    );
  }
  ...
  if (!user) {
    return Response.json(
      { error: 'Please sign in to get a hint.' },
      { status: 401 },
    );
  }
  ...
  try {
    const hint = await generateHint(prompt, context);
    return Response.json({ hint });
  } catch (err) {
    const message =
      err instanceof Error
        ? err.message
        : 'Something went wrong while creating a hint.';
