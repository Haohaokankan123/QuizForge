...
STEP 0 — FIRST, IDENTIFY WHAT KIND OF MATERIAL THIS IS (decide before writing anything):
- VOCABULARY / TERMINOLOGY LIST: the source is mostly entries like "Word (part of speech) definition", or a glossary of terms. ➜ EVERY question MUST test a listed WORD/TERM itself — its meaning, or using the right word for a situation (meaning→word, word→meaning, or fill-in where the answer is a listed word). The example sentences that come with a word are ONLY there to illustrate it — NEVER ask a question about what happened in an example sentence. (CRITICAL ANTI-EXAMPLE: if an entry is "scarce — example: 'Tacos were scarce on Tuesday', " a question like "Tacos were scarce on Tuesday because they were priced higher — True/False?" is WRONG and FORBIDDEN: it tests the example's story, not the word 'scarce'. The RIGHT question tests the word: "Which word means 'not enough to meet demand'?" → scarce, or "After the bakery sold out by noon, fresh bread became ____." → scarce.) The answer to every question is ALWAYS one of the listed words/terms (or its meaning).
- MATH / FORMULAS: ➜ write WORD PROBLEMS (or pure computations) the student must SOLVE using the source's formulas/values. Test the math, not trivia about the wording.
- PROCEDURE / PROCESS / SCIENCE EXPLANATION: ➜ test the steps, cause/effect, and what-happens-if, not incidental detail.
- ARTICLE / PROSE / NOTES (no list structure): ➜ test the key ideas, claims, and relationships the passage actually teaches — not throwaway details (a name dropped once, a number in a passing example) unless that detail IS the point.

IRON RULE (all types): test the THING BEING TAUGHT, never the illustration used to teach it. An example sentence, a sample story, or a "for instance…" is scaffolding — quiz the concept/word/formula it demonstrates, not the made-up details inside the example. If a candidate question could be answered without knowing the actual word/concept/formula being studied, it is the WRONG question — replace it.
